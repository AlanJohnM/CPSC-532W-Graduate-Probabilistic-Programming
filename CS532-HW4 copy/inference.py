...
def aggregate_samples(samples, program, algorithm, graph=None, plot=False):
    if program == '1':
        if algorithm == 'likelihood_weighting':
            ...
            if plot:
                plt.hist(results, weights=[w.item() for w in torch.softmax(torch.tensor(weights), dim=0)], bins=100, range=(mean-5,mean+5))
                plt.title("Posterior for Mu via BBVI")
                plt.savefig("figures/1_hist_mu_BBVI.png")
                plt.clf()
                
                weights = weights[::100]
                plt.plot(weights)
                plt.title("Trace of ELBO of Mu via BBVI")
                plt.savefig("figures/1_elbo.png")
                plt.clf()
        ...
        if algorithm == 'likelihood_weighting':
            ...
            for i in range(2):
                ...
                if plot:
                    plt.hist(results, weights=[w.item() for w in torch.softmax(torch.tensor(weights), dim=0)], bins=100, range=(mean-5,mean+5))
                    plt.title(f"Posterior for {params[i]} via BBVI")
                    plt.savefig(f"figures/2_hist_{params[i]}_BBVI.png")
                    plt.clf()
            
            weights = weights[::100]
            plt.plot(weights)
            plt.title(f"Trace of ELBO via BBVIA")
            plt.savefig(f"figures/2_elbo.png")
            plt.clf()
        ...
        if algorithm == 'likelihood_weighting':
            ...
            weights = weights[::100]
            plt.plot(weights)
            plt.title(f"Trace of ELBO via BBVI")
            plt.savefig(f"figures/3_elbo.png")
            plt.clf()

#            params = ['z1','z2','z3','z4','z5','z6','z7']
#            weights = [w.item() for w,m in samples]
#            weights = torch.softmax(torch.tensor(weights), dim = 0)
#            expectation = 0.0
#            for i, (_, res) in enumerate(samples):
#                expectation += weights[i] if res[0] == res[1] else 0.0
#            print(expectation)
#            for i in range(7):
##                results = [m[i].item() for w,m in samples]
#
...
#                print(f"Weighted Expectation of {params[i]}: {mean}")
        ...
        elif algorithm == 'bbvi':#different program 4 here
            params = ['W_0', 'b_0', 'W_1', 'b_1']
            weights_plot = weights = [w.item() for w,m in samples][::100]
            for i in range(len(params)):
                results = [m[i] for w,m in samples]
                weights = [w.item() for w,m in samples]
                weights = torch.softmax(torch.tensor(weights), dim=0)
                heatmap = torch.zeros_like(results[0])
                for res, weight in zip(results, weights):
                    heatmap += res*weight
                
                plt.imshow(heatmap, interpolation='nearest', cmap='hot')
                plt.title(f"Heatmap of distribution of {params[i]}")
                plt.savefig(f"figures/heatmap_{params[i]}")
                plt.clf()
            
            plt.plot(weights_plot)
            plt.title(f"Trace of ELBO via BBVI")
            plt.savefig(f"figures/4_elbo.png")
            plt.clf()
        ...
        for i in range(2):
            if algorithm == 'likelihood_weighting':
            ...
            elif algorithm == 'bbvi': #different program here
                results = [m.item() for w,m in samples]
                weights = [w.item() for w,m in samples]
                ...
                print(f"Weighted Expectation of s: {mean}")
                print(f"Weighted Variance of s: {var}")
                
                if plot:
                    plt.hist(results, weights=[w.item() for w in torch.softmax(torch.tensor(weights), dim=0)], bins=100, range=(mean-5,mean+5))
                    plt.title("Posterior for s via BBVI")
                    plt.savefig("figures/5_hist_s_BBVI.png")
                    plt.clf()
                    
                    weights = weights[::100]
                    plt.plot(weights)
                    plt.title("Trace of ELBO of s via BBVI")
                    plt.savefig("figures/5_elbo.png")
                    plt.clf()
