...
def EVAL(e, sigma, l, rho):
    if type(e) != list or len(e) == 1:
        ...
        d_1, sigma = EVAL(e[1], sigma, l, rho)
        c_2, sigma = EVAL(e[2], sigma, l, rho)
        sigma['log_W'] = sigma['log_W'] + d_1.observe(c_2)
        return c_2, sigma
...
def likelihood_weighting(ast, L):
    ...
    for l in tqdm(range(L)):
        sigma = {'log_W':0}
        r, sigma = evaluate_program(ast,sigma=sigma)
        log_W = sigma['log_W']
        samples.append((log_W, r))
    return samples
