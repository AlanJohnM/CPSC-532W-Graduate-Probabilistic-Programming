...
def leap_phrog(chi,Y_cal,P,R,T,eps,):
    ...
    for i in range(T):
        chi = {k:enable_grad(v + eps*R_half[i]) for i, (k, v) in enumerate(chi.items())}
...
def hamiltonian_monte_carlo(graph, L):
    ...
    samples = []
    chi = {x:enable_grad(s) for x,s in zip(X,sample_from_joint(graph[0:2] + [['vector'] + X]))}
    ...
    for i in tqdm(range(L + burn)):
        R = kick.sample()
        chi_base = {k:enable_grad(torch.clone(v)) for k,v in chi.items()}
        ...
        if u < alpha:
            chi = chi_prime
#        if i > burn and  i%thin == 0: # need all samples for the trace sigh
