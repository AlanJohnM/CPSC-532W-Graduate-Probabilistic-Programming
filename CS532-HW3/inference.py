...
from evaluation_based_likelihood_weighting import likelihood_weighting
from graph_based_mh_in_gibbs import metropolis_hastings_in_gibbs
from graph_based_mh_in_gibbs import deterministic_eval as gibbs_eval
from graph_based_mh_in_gibbs import burn as gibbs_burn
from graph_based_hmc import hamiltonian_monte_carlo
from graph_based_hmc import deterministic_eval as hmc_eval
from graph_based_hmc import burn as hmc_burn
...
def aggregate_samples(samples, program, algorithm, graph=None):
    if program == '1':
        if algorithm == 'likelihood_weighting':
            ...
            plt.savefig("figures/1_trace_low_w_hmc.png")
            plt.clf()
        ...
        if algorithm == 'likelihood_weighting':
            ...
            plt.savefig("figures/3_hist_rain_IS.png")
            plt.clf()
            ...
            plt.savefig("figures/4_trace_equality_mhg.png")
            plt.clf()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-A', '--algorithm', required=False, default='likelihood_weighting', type=str)
    args = parser.parse_args()
    
    if args.algorithm not in algorithms.keys():
        raise Error("Unknown Inference Algorithm Selected")
    evaluator = "desugar" if args.algorithm == "likelihood_weighting" else "graph"
    inference_algorithm = algorithms[args.algorithm]
    
    for p in args.programs:
        program = daphne([f'{evaluator}', '-i', f'../CS532-HW3/programs/{p}.daphne'])
        samples = inference_algorithm(program, args.n_samples)
        aggregate_samples(samples, p, args.algorithm, program)
